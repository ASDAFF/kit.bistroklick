...
//mprint($arResult);
            ...
            <div class="imager">
                <div class="imageWp">
                    ...
                    if ($arResult["TOVAR"]["DETAIL_PICTURE"]) {
                        ...
                        <img class="item_img" itemprop="image" src="<?= $src ?>"
                             alt="<?= $arResult["TOVAR"]["NAME"] ?>"/>
                    <?
                    } else {
                        ?>
                ...
                <div class="pricen">
                    <div class="priceFirst">
                        <?
                        if ($arResult["OFFERS"]) {
                            echo GetMessage('KIT_CAT_BK_OT') . ' ' . CurrencyFormat(round($arResult['PRICE_MIN']), $arParams['CURRENCY_ID']);
                        } else {
        ...
        <div class="mlfform">
            ...
            if ($arResult['OFFERS']) {
                ...
                foreach ($arResult['OFFERS'] as $key => $offer) {
                    ...
                    if (($key == 0 && !$_REQUEST['offer']) || $offer['ID'] == intval($_REQUEST['offer'])) echo " checked"; ?>/>
                    <?
            ...
            if (is_array($arParams['FIELD_SHOW']) && count($arParams['FIELD_SHOW']) > 0) {
                foreach ($arParams['FIELD_SHOW'] as $value) {
                    ...
                    if (isset($arResult['LABEL'][$value])) {
                        ...
                        $fieldname = GetMessage("KIT_CAT_BK_FIELD_REQ_" . strtoupper($value));
                    }
                    ?><?
                    if ($value == 'mess') {
                        $messfield = true;
                        ...
                        ?>
                        <div class="field"><label for="<?= $value ?>"><?= $fieldname ?><?
                            if ($arResult['SEND_REQ'][$value] == 1) echo '<span>*</span>'; ?>:</label>
                        <?
                        if ($value != 'addfield_' . $arResult['LOC_ID']) {
                            ?>
                            <input id="<?= $value ?>" type="text" name="<?= $value ?>"
                                   value="<?= $arResult['SEND'][$value] ?>"/>
                        <?
                        } else {
                            ?>
            ...
            if (is_array($arParams["FIELD_DELIVERY"]) && count($arParams["FIELD_DELIVERY"]) == 1) {
                ...
                ?>
                <div class="field"><label for="delivery"><?= GetMessage("KIT_CAT_BK_DELIVERY") ?></label>
                <div class="locationwrap"><select id="delivery" name="delivery">
                        ...
                        foreach ($arParams["FIELD_DELIVERY"] as $key => $val) {
                            ...
                            ?>
                            <option value="<?= $val ?>"<?
                            if (($i == 1 && $arResult['SEND']['delivery'] == "") || $arResult['SEND']['delivery'] != "") {
                                echo ' selected';
                            } ?>><?= $arResult['DELIVERY_NAME'][$val] ?></option><?
                        } ?>
                    </select></div>
                </div><?
            } ?>
            ...
            if (is_array($arParams["FIELD_PAYSYSTEM"]) && count($arParams["FIELD_PAYSYSTEM"]) == 1) {
                ?><input type="hidden" name="paysystem" value="<?
                ...
                ?>
                <div class="field"><label for="paysystem"><?= GetMessage("KIT_CAT_BK_PAYSYSTEM") ?></label>
                <div class="locationwrap"><select id="paysystem" name="paysystem">
                        ...
                        foreach ($arParams["FIELD_PAYSYSTEM"] as $key => $val) {
                            ...
                            ?>
                            <option value="<?= $val ?>"<?
                            if (($i == 1 && $arResult['SEND']['paysystem'] == "") || $arResult['SEND']['paysystem'] != "") {
                                echo ' selected';
                            } ?>><?= $arResult['PAYSYSTEM_NAME'][$val] ?></option><?
                        } ?>
                    </select></div>
                </div><?
            } ?>
            <?
            if ($messfield) { ?>
                <div class="field"><label for="mess"><?= GetMessage("KIT_CAT_BK_FIELD_REQ_" . strtoupper('mess')) ?><?
                        if ($arResult['SEND_REQ']['mess'] == 1) echo '<span>*</span>'; ?>:</label>
    ...
    <body>
    <? echo GetMessage('KIT_CAT_BK_ERROR_KEY'); ?>
    </body>
    </html>
    <?
    die();
